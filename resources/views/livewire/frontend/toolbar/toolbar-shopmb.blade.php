<!-- toolbarShopmb -->
<div class="offcanvas offcanvas-start canvas-mb toolbar-shop-mobile" id="toolbarShopmb">
    <span class="icon-close icon-close-popup" data-bs-dismiss="offcanvas" aria-label="Close"></span>
    <div class="mb-canvas-content">
        <div class="mb-body">
            <ul class="nav-ul-mb" id="wrapper-menu-navigation">
                <li class="nav-mb-item">
                    <a href="{{url('category/id=1')}}" class="tf-category-link mb-menu-link">
                        <div class="image">
                            <img src="{{asset('assets/images/shop/cate/cate1.jpg')}}" alt="">
                        </div>
                        <span>Accessories</span>
                    </a>
                </li>
                <li class="nav-mb-item">
                    <a href="{{url('category/id=1')}}" class="tf-category-link mb-menu-link">
                        <div class="image">
                            <img src="{{asset('assets/images/shop/cate/cate2.jpg')}}" alt="">
                        </div>
                        <span>Dog</span>
                    </a>
                </li>
                <li class="nav-mb-item">
                    <a href="{{url('category/id=1')}}" class="tf-category-link mb-menu-link">
                        <div class="image">
                            <img src="{{asset('assets/images/shop/cate/cate3.jpg')}}" alt="">
                        </div>
                        <span>Grocery</span>
                    </a>
                </li>
                <li class="nav-mb-item">
                    <a href="{{url('category/id=1')}}" class="tf-category-link mb-menu-link">
                        <div class="image">
                            <img src="{{asset('assets/images/shop/cate/cate4.png')}}" alt="">
                        </div>
                        <span>Handbag</span>
                    </a>
                </li>
                <li class="nav-mb-item">
                    <a href="#cate-menu-one" class="tf-category-link has-children collapsed mb-menu-link" data-bs-toggle="collapse" aria-expanded="true" aria-controls="cate-menu-one">
                        <div class="image">
                            <img src="{{asset('assets/images/shop/cate/cate5.jpg')}}" alt="">
                        </div>
                        <span>Fashion</span>
                        <span class="btn-open-sub"></span>
                    </a>
                    <div id="cate-menu-one" class="collapse list-cate">
                        <ul class="sub-nav-menu" id="cate-menu-navigation">
                            <li>
                                <a href="#cate-shop-one" class="tf-category-link has-children sub-nav-link collapsed"  data-bs-toggle="collapse" aria-expanded="true" aria-controls="cate-shop-one">
                                    <div class="image">
                                        <img src="{{asset('assets/images/shop/cate/cate6.jpg')}}" alt="">
                                    </div>
                                    <span>Mens</span>
                                    <span class="btn-open-sub"></span>
                                </a>
                                <div id="cate-shop-one" class="collapse">
                                    <ul class="sub-nav-menu sub-menu-level-2">
                                        <li>
                                            <a href="{{url('category/id=1')}}" class="tf-category-link sub-nav-link">
                                                <div class="image">
                                                    <img src="{{asset('assets/images/shop/cate/cate1.jpg')}}" alt="">
                                                </div>
                                                <span>Accessories</span>
                                            </a>
                                        </li>
                                        <li>
                                            <a href="{{url('category/id=1')}}" class="tf-category-link sub-nav-link">
                                                <div class="image">
                                                    <img src="{{asset('assets/images/shop/cate/cate8.jpg')}}" alt="">
                                                </div>
                                                <span>Shoes</span>
                                            </a>
                                        </li>
                                    </ul>
                                </div>
                            </li>
                            <li>
                                <a href="#cate-shop-two" class="tf-category-link has-children sub-nav-link collapsed"  data-bs-toggle="collapse" aria-expanded="true" aria-controls="cate-shop-two">
                                    <div class="image">
                                        <img src="{{asset('assets/images/shop/cate/cate9.jpg')}}" alt="">
                                    </div>
                                    <span>Womens</span>
                                    <span class="btn-open-sub"></span>
                                </a>
                                <div id="cate-shop-two" class="collapse">
                                    <ul class="sub-nav-menu sub-menu-level-2">
                                        <li>
                                            <a href="{{url('category/id=1')}}" class="tf-category-link sub-nav-link">
                                                <div class="image">
                                                    <img src="{{asset('assets/images/shop/cate/cate4.png')}}" alt="">
                                                </div>
                                                <span>Handbag</span>
                                            </a>
                                        </li>
                                        <li>
                                            <a href="{{url('category/id=1')}}" class="tf-category-link sub-nav-link">
                                                <div class="image">
                                                    <img src="{{asset('assets/images/shop/cate/cate7.jpg')}}" alt="">
                                                </div>
                                                <span>Tee</span>
                                            </a>
                                        </li>
                                    </ul>
                                </div>
                            </li>
                        </ul>
                    </div>
                </li>
                <li class="nav-mb-item">
                    <a href="#cate-menu-two" class="tf-category-link has-children collapsed mb-menu-link" data-bs-toggle="collapse" aria-expanded="true" aria-controls="cate-menu-two">
                        <div class="image">
                            <img src="{{asset('assets/images/shop/cate/cate6.jpg')}}" alt="">
                        </div>
                        <span>Men</span>
                        <span class="btn-open-sub"></span>
                    </a>
                    <div id="cate-menu-two" class="collapse list-cate">
                        <ul class="sub-nav-menu" id="cate-menu-navigation1">
                            <li>
                                <a href="{{url('category/id=1')}}" class="tf-category-link sub-nav-link">
                                    <div class="image">
                                        <img src="{{asset('assets/images/shop/cate/cate1.jpg')}}" alt="">
                                    </div>
                                    <span>Accessories</span>
                                </a>
                            </li>
                            <li>
                                <a href="{{url('category/id=1')}}" class="tf-category-link sub-nav-link">
                                    <div class="image">
                                        <img src="{{asset('assets/images/shop/cate/cate8.jpg')}}" alt="">
                                    </div>
                                    <span>Shoes</span>
                                </a>
                            </li>
                        </ul>
                    </div>
                </li>
                <li class="nav-mb-item">
                    <a href="{{url('category/id=1')}}" class="tf-category-link mb-menu-link">
                        <div class="image">
                            <img src="{{asset('assets/images/shop/cate/cate7.jpg')}}" alt="">
                        </div>
                        <span>Tee</span>
                    </a>
                </li>
                <li class="nav-mb-item">
                    <a href="{{url('category/id=1')}}" class="tf-category-link mb-menu-link">
                        <div class="image">
                            <img src="{{asset('assets/images/shop/cate/cate8.jpg')}}" alt="">
                        </div>
                        <span>Shoes</span>
                    </a>
                </li>
                <li class="nav-mb-item">
                    <a href="#cate-menu-three" class="tf-category-link has-children collapsed mb-menu-link" data-bs-toggle="collapse" aria-expanded="true" aria-controls="cate-menu-three">
                        <div class="image">
                            <img src="{{asset('assets/images/shop/cate/cate9.jpg')}}" alt="">
                        </div>
                        <span>Women</span>
                        <span class="btn-open-sub"></span>
                    </a>
                    <div id="cate-menu-three" class="collapse list-cate">
                        <ul class="sub-nav-menu" id="cate-menu-navigation2">
                            <li>
                                <a href="{{url('category/id=1')}}" class="tf-category-link sub-nav-link">
                                    <div class="image">
                                        <img src="{{asset('assets/images/shop/cate/cate4.png')}}" alt="">
                                    </div>
                                    <span>Handbag</span>
                                </a>
                            </li>
                            <li>
                                <a href="{{url('category/id=1')}}" class="tf-category-link sub-nav-link">
                                    <div class="image">
                                        <img src="{{asset('assets/images/shop/cate/cate7.jpg')}}" alt="">
                                    </div>
                                    <span>Tee</span>
                                </a>
                            </li>
                        </ul>
                    </div>
                </li>
            </ul>
        </div>
        <div class="mb-bottom">
            <a href="{{url('category/id=1')}}" class="tf-btn fw-5 btn-line">View all collection<i class="icon icon-arrow1-top-left"></i></a>
        </div>
    </div>
</div>
<!-- /toolbarShopmb -->
